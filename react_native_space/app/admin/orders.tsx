  ...
  if (loading) {
    return (
      <View style={styles.container}>
        {[...Array(5)].map((_, i) => (
          <SkeletonBox key={i} height={80} style={{ margin: Spacing.md }} />
        ))}
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>📋 {t('manageOrders')}</Text>
      </View>
            ...
            <TouchableOpacity style={styles.card} onPress={() => updateStatus(item.id, item.status)}>
              <View style={styles.row}>
                ...
                <View style={[styles.badge, { backgroundColor: (STATUS_COLOR[item.status] ?? Colors.warning) + '33' }]}>
                  <Text style={[styles.badgeText, { color: STATUS_COLOR[item.status] ?? Colors.warning }]}>
                    {item.status}
                  </Text>
                </View>
              </View>
              <View style={styles.row}>
                <Text style={styles.meta}>
                  {item.user?.firstName} {item.user?.lastName} • {format(new Date(item.createdAt), 'dd MMM HH:mm')}
                </Text>
