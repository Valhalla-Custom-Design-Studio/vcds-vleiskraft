...
export default function OrderDetailScreen() {
  ...
  if (loading) return <View style={styles.container}><SkeletonBox height={400} style={{ margin: Spacing.md }} /></View>;
  if (!order) return <View style={styles.container}><Text style={{ color: Colors.textPrimary, padding: Spacing.md }}>Order not found</Text></View>;
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.header}>
        ...
        <View style={[styles.badge, { backgroundColor: (STATUS_COLOR[order.status] ?? Colors.warning) + '33' }]}>
          <Text style={[styles.badgeText, { color: STATUS_COLOR[order.status] ?? Colors.warning }]}>{order.status}</Text>
        </View>
      </View>
      <Text style={styles.date}>{format(new Date(order.createdAt), 'dd MMM yyyy HH:mm')}</Text>
      <GlassCard style={styles.card}>
        <Text style={styles.cardTitle}>{t('items', language)}</Text>
        ...
        <View style={styles.totalRow}>
          <Text style={styles.totalLabel}>{t('total', language)}</Text>
          <Text style={styles.totalAmount}>R{order.totalAmount?.toFixed(2)}</Text>
        </View>
      </GlassCard>
      {order.deliveryType === 'DELIVERY' && order.status !== 'DELIVERED' && (
        <TouchableOpacity style={styles.trackBtn} onPress={() => router.push(`/delivery-tracking/${orderId}`)}>
          <Ionicons name="location-outline" size={20} color={Colors.primary} />
          <Text style={styles.trackBtnText}>{t('deliveryTracker', language)}</Text>
        </TouchableOpacity>
      )}
      <GradientButton label={t('orderAgain', language)} onPress={reorder} style={{ marginBottom: Spacing.md }} />
      {(order.status === 'DELIVERED' || order.status === 'COLLECTED') && !rated && (
        <GlassCard style={styles.card}>
          <Text style={styles.cardTitle}>{t('rateOrder', language)}</Text>
          <Text style={styles.rateQ}>{t('howWasOrder', language)}</Text>
          ...
          <TextInput style={styles.commentInput} placeholder={t('commentOptional', language)} placeholderTextColor={Colors.textSecondary} value={comment} onChangeText={setComment} multiline />
          <GradientButton label={submitting ? t('loading', language) : t('submitRating', language)} onPress={submitRating} loading={submitting} />
        </GlassCard>
      )}
