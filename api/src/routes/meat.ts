...
router.get("/catalogue", authenticate, (_req: Request, res: Response) => {
  res.status(200).json({ items: [], message: "Meat catalogue — coming soon" });
});

router.get("/cuts", authenticate, (_req: Request, res: Response) => {
  res.status(200).json({ cuts: [], message: "Meat cuts database — coming soon" });
});

router.post("/order", authenticate, (_req: Request, res: Response) => {
